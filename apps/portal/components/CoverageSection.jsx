...
function CoverageSection() {
  ...
  return (
    <section className="coverage-section">
      <h2 className="section-title">{t('覆盖全球60+国家，在线可用率高达99.99%')}</h2>

      <div className="coverage-features">
        <div className="feature-item">{t('支持多种场景')}</div>
        <div className="feature-divider">|</div>
        <div className="feature-item">{t('全协议')}</div>
        <div className="feature-divider">|</div>
        <div className="feature-item">{t('10G+高速带宽')}</div>
        <div className="feature-divider">|</div>
        <div className="feature-item">{t('按需计费方案')}</div>
      </div>

      <div className="world-map">
        <img alt={t('世界地图')} className="map-image" src="/assets/map-B2s9UNF1.png" />
      </div>

      <div className="country-list">
        {countries.map((country, index) => (
          <div key={index} className={`country-item ${country.soldOut ? 'oos-item' : ''}`}>
            <img alt={t(country.name)} className="country-flag-img" src={country.flag} />
            <div className="country-info">
              <div className="country-name">{t(country.name)}</div>
              ...
              <>
                <div className="oos-hover-layer">
                  <div className="oos-icon-wrapper">
                    ...
                    <span className="oos-icon-label">{t('已售罄')}</span>
                  </div>
        ...
        <div className="country-item more-countries">
          <div className="more-link">{t('查看更多地区')}</div>
        </div>
      </div>
    </section>
  );
